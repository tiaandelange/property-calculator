-- Support INV-YYMM-{####} and INV-{####} invoice number formats in generate_invoice_number.
...
BEGIN
  ...
  v_yymm := to_char(timezone('UTC', now()), 'YYMM');

  IF v_format = 'INV-{####}' THEN
    SELECT coalesce(max(
      CASE
        WHEN i.invoice_number ~ '^INV-[0-9]{4}$' THEN
  ...
  ELSIF v_format = 'INV-YYMM-{####}' THEN
    SELECT coalesce(max(
      CASE
        WHEN i.invoice_number ~ ('^INV-' || v_yymm || '-[0-9]{4}$') THEN
          substring(i.invoice_number from '[0-9]{4}$')::integer
        ELSE NULL
      END
    ), 0) + 1
  ...
  v_result := replace(v_result, 'YYMM', v_yymm);
...
COMMENT ON FUNCTION public.generate_invoice_number (uuid) IS
  'User-scoped invoice numbers from user_settings.invoice_number_format (INV-YY-{####}, INV-{####}, INV-YYMM-{####}, INV-YYYY-{####}).';
