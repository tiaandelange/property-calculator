-- Statement invoice lines: enriched RPC rows + statement_lines view (invoice_id link, no duplicate storage).

CREATE OR REPLACE VIEW public.statement_lines
...
SELECT
  ...
  i.unit_id,
  i.lease_id,
  i.tenant_id AS primary_tenant_id,
  CASE
    WHEN i.invoice_type = 'RENT'::public.app_invoice_type THEN 'rent_invoice'
    WHEN i.invoice_type = 'UTILITY_RECOVERY'::public.app_invoice_type THEN 'utility_recovery_invoice'
    ELSE 'invoice'
  END AS statement_type,
  (coalesce(i.due_date, i.issue_date, i.invoice_date))::date AS line_date,
  i.invoice_number AS reference,
  coalesce(i.total_amount, i.total) AS credit,
...
WHERE i.status NOT IN ('CANCELLED'::public.app_invoice_status, 'VOID'::public.app_invoice_status)
  AND i.archived_at IS NULL;

COMMENT ON VIEW public.statement_lines IS
  'Read-only statement registry: one row per invoice (invoice_id unique). Property/tenant statements read via get_property_monthly_statement.';

GRANT SELECT ON public.statement_lines TO authenticated;
      ...
      CASE
        WHEN inv.invoice_type = 'RENT'::public.app_invoice_type THEN
          concat (
            ...
            CASE
              WHEN pu.unit_label IS NOT NULL AND trim(pu.unit_label) <> '' THEN concat(' / ', pu.unit_label)
              WHEN pu.unit_number IS NOT NULL AND trim(pu.unit_number) <> '' THEN concat(' / Unit ', pu.unit_number)
              ELSE ''
            END,
            ' — ',
