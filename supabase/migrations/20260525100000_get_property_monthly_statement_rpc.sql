-- Monthly property statement (workspace Financials / Overview).
-- Mirrors Express GET /properties/:id/statement + buildPropertyStatement / bondFinance / deposits /
-- futureCharges / recurringCharges from backend/src/domains/properties/property.statement.service.ts
-- and ownedPropertiesRoutes.ts (excluding materializeDueRecurringExpenses + applyDepositGrowthForCurrentPropertyLeases).
...
BEGIN
  ...
  inv_rows AS (
    SELECT
      ...
      to_char (inv.invoice_date AT TIME ZONE 'UTC', 'YYYY-MM-DD') AS d,
      CASE
      ...
      CASE
        ...
        ELSE concat ('Invoice (', inv.status::text, ') — unpaid')
      END AS typ,
      NULL::numeric AS debit,
      round(inv.total::numeric, 2) AS credit,
      'INVOICE'::text AS src,
  ...
  WHERE
    inv.property_id = p_property_id
    AND inv.user_id = v_uid
    AND inv.status = ANY (ARRAY['DRAFT', 'SENT', 'OVERDUE']::app_invoice_status[])
    AND (inv.archived_at IS NULL);
...
REVOKE ALL ON FUNCTION public.get_property_monthly_statement (uuid, integer, integer, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_property_monthly_statement (uuid, integer, integer, boolean) TO authenticated;

COMMENT ON FUNCTION public.get_property_monthly_statement (uuid, integer, integer, boolean) IS
  'Workspace monthly statement JSON; auth.uid(); see MIGRATION_STATUS parity vs Express.';
