-- Co-applicant names/emails/phones are combined only on the applicants tab.
-- Tenants tab, workspace, and lease pickers show each person as a separate row.
...
COMMENT ON FUNCTION public.get_tenants_directory (integer, integer, text, uuid, text, text, text) IS
  'Tenants/applicants directory. Applicants tab merges co-applicants into one row; tenants tab lists each person separately.';
