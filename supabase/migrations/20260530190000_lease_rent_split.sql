-- Per-tenant rent share on multi-tenant leases (sums to leases.monthly_rent when split).

ALTER TABLE public.leases
  ADD COLUMN IF NOT EXISTS rent_split_enabled boolean NOT NULL DEFAULT false;

ALTER TABLE public.lease_tenants
  ADD COLUMN IF NOT EXISTS rent_share_amount double precision;

COMMENT ON COLUMN public.leases.rent_split_enabled IS
  'When true, lease_tenants.rent_share_amount values sum to monthly_rent for invoicing/display.';

COMMENT ON COLUMN public.lease_tenants.rent_share_amount IS
  'Optional share of lease monthly_rent for this tenant when rent_split_enabled is true.';
...
BEGIN
  ...
  INSERT INTO public.leases (
    ...
    escalation_percent, escalation_date, status, lease_document_id, notes
  )
  VALUES (
