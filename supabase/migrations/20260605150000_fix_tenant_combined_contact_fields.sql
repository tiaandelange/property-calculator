-- Split legacy "email1 & email2" / phone values stored on individual tenant rows.
-- Directory tenants tab must never surface merged co-applicant contact fields.

CREATE OR REPLACE FUNCTION public.tenant_contact_field (p_value text, p_role text)
  RETURNS text
  LANGUAGE sql
  IMMUTABLE
  AS $$
  SELECT
    ...
    WHEN position(' & ' IN p_value) = 0 THEN
      btrim(p_value)
    WHEN coalesce(p_role, 'PRIMARY') = 'CO' THEN
      nullif(btrim(split_part(p_value, ' & ', 2)), '')
    ELSE
      nullif(btrim(split_part(p_value, ' & ', 1)), '')
    END;
$$;

COMMENT ON FUNCTION public.tenant_contact_field (text, text) IS
  'Returns one person''s email/phone; splits legacy "primary & co" combined values by applicant_group_role.';

-- Repair rows polluted before per-person contact fields were enforced.
UPDATE
  public.tenants t
SET
  email = public.tenant_contact_field(t.email, coalesce(t.applicant_group_role, 'PRIMARY')),
  phone = public.tenant_contact_field(t.phone, coalesce(t.applicant_group_role, 'PRIMARY')),
  updated_at = now()
WHERE
  t.email LIKE '% & %'
  OR t.phone LIKE '% & %';

-- get_tenants_directory: per-person contact on tenants tab; combined only on applicants tab.
