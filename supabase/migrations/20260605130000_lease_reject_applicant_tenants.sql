-- Leases may only reference active tenants, not applicants (status = APPLICANT).
...
BEGIN
  ...
  FOR v_tenant IN SELECT * FROM jsonb_array_elements(v_lease_tenants) LOOP
    ...
    WHERE t.id = v_tid
      AND t.user_id = v_uid
      AND t.status = 'APPLICANT'::public.app_tenant_status;
    IF FOUND THEN
      RAISE EXCEPTION 'APPLICANT_NOT_ELIGIBLE_FOR_LEASE';
    END IF;
...
COMMENT ON FUNCTION public.create_property_lease (jsonb) IS
  'Creates a lease for an owned property; rejects applicants (promote to tenant first).';
